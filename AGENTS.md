...
- **`agent_type: 'explore'`** selects the read-only Codex explore recipe; any other value (including omit) uses the general Codex agent.
- **Every request sets `agentOptions` / `persona` / `toolFilter` / `maxDepth: 3` explicitly**, because the continuable route never calls `provider.start()` (mesh AGENTS.md §3.2). `agentOptions` is always `{ provider: 'codex-kernel', model: args.model || 'deepseek-v4-flash:0731' }`. Persona and `toolFilter` use Codex-surface tool names (`exec_command`, `apply_patch`, `view_file`, …), not the mesh recipe's generic DSH names.
- **Provider preference** is `codex-agent` / `codex-explore` when listed, then `codex-agent`, then `spawn`.
