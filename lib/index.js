...
async function apply(ctx) {
    ...
    register(strDef({
      name: 'update_plan',
      description: 'Updates the task plan (a TODO/checklist). Provide an optional explanation and a list of plan items, each with a step and status (pending | in_progress | done).',
      parameters: {
        type: 'object',
        properties: {
          ...
          plan: {
            ...
            items: {
              ...
              properties: {
                ...
                status: { type: 'string', enum: ['pending', 'in_progress', 'done'], description: 'The step status.' },
              },
      ...
      execute: async (args, exec) => {
        ...
        if (Array.isArray(args.plan)) planStore.set(key, args.plan)
    ...
    // Codex's built-in subagent types, self-contained: persona (distilled
    // from the Codex / mesh recipe prompts) and toolFilter (Codex-surface tool
    ...
    const CODEX_SUBAGENTS = {
      agent: {
        provider: 'codex-agent',
        persona: 'You are Codex, a coding agent running as a subagent of a main agent. All user messages come from the parent agent, which only sees your final message; treat the parent as your caller and do not ask the end user questions directly. Complete the assigned task with tools (exec_command, apply_patch, view_file, write_file, edit_file, glob, grep), then return a compact, technically complete summary.',
        toolFilter: { allow: ['exec_command', 'apply_patch', 'view_file', 'write_file', 'edit_file', 'glob', 'grep', 'web_search', 'view_image', 'update_plan', 'request_user_input', 'list_tasks', 'task_output', 'task_stop', 'sleep'] },
      },
      explore: {
        provider: 'codex-explore',
        persona: 'You are a Codex exploration subagent: fast, read-only codebase exploration. You have NO file editing tools. Use glob/grep/view_file; run exec_command only for read-only commands (ls, git status, git log, git diff, find). Report findings in a structured format with absolute paths.',
        toolFilter: { allow: ['exec_command', 'view_file', 'glob', 'grep', 'web_search', 'view_image'] },
      },
    }
    ...
    register(strDef({
      ...
      parameters: {
        ...
        properties: {
          ...
          agent_type: { type: 'string', description: 'Agent type override for the new agent. Omit to inherit the parent agent type. DSH form maps "explore" onto the read-only Codex explore recipe; any other value (including omit) uses the general Codex agent.' },
      ...
      execute: async (args, exec) => {
        if (!subagents) return 'Error: subagents service unavailable.'
        if (!exec.agent) return 'Error: no caller agent.'
        const spec = CODEX_SUBAGENTS[args.agent_type === 'explore' ? 'explore' : 'agent']
        const names = subagents.list()
        let providerName = names.indexOf(spec.provider) >= 0 ? spec.provider : (names.indexOf('codex-agent') >= 0 ? 'codex-agent' : (names.indexOf('spawn') >= 0 ? 'spawn' : null))
        ...
        const request = {
          ...
          parent: exec.agent,
          agentOptions: { provider: 'codex-kernel', model: args.model || 'deepseek-v4-flash:0731' },
    ...
    // ---- plan mode (enter_plan_mode / exit_plan_mode equivalences) ----
    // Codex 0.146.0 models plan mode as `update_plan` + review; it has no
    // enter_plan_mode/exit_plan_mode tool. We expose the DSH planMode service
    // under codex-flavoured names only when planMode is present.
    if (planMode) {
      register(strDef({
        name: 'enter_plan_mode',
        description: 'Enters plan mode: from now on plan the implementation instead of executing it, using only non-mutating tools until exit_plan_mode succeeds. (Codex-compatible alias over the DSH planMode service; the installed Codex uses `update_plan` for planning instead of a dedicated enter/exit pair.)',
        parameters: { type: 'object', properties: {} },
        execute: async (args, exec) => {
          if (!exec.agent) return 'Error: no caller agent.'
          return 'Plan mode entered (' + planMode.set(exec.agent, true) + ').'
        },
      }))
      register(strDef({
        name: 'exit_plan_mode',
        description: 'Exits plan mode. Call once the plan is ready; the plan itself is presented in your response text.',
        parameters: { type: 'object', properties: {} },
        execute: async (args, exec) => {
          if (!exec.agent) return 'Error: no caller agent.'
          return 'Plan mode exited (' + planMode.set(exec.agent, false) + ').'
        },
      }))
    }
